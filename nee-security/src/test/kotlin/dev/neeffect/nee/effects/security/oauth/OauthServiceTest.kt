...
internal class OauthServiceTest : DescribeSpec({
    ...
    describe("oauth service") {
        ...
        describe("login to google") {
            val result = service.login("acode", GoogleOpenIdTest.preservedState, OauthProviderName.Google)
                .perform(Unit)(Unit)
            ...
            it ("should contain user id in token") {
                ...
                testModule.jwtConfigModule.jwtCoder.decodeJwt(jwt).get().subject shouldBe "ba419d35-0dfe-8af7-aee7-bbe10c45c028"
            }
        ...
        describe("login with bad state") {
            val result = service.login("acode", "really bad state", OauthProviderName.Google)
                .perform(Unit)(Unit)
            it ("should fail") {
    ...
    companion object {
        ...
        const val   expectedUrlCall = "https://accounts.google.com/o/oauth2/v2/auth?response_type=code&client_id=testId&scope=openid%20profile%20email%20https://www.googleapis.com/auth/user.organization.read&redirect_uri=http://globalpost.any/mypath&state=5DwIT0u7K/GDne5GbYUstQ==@Z0GCZibsGi2m32XO2BO3J/i7GyncfvHwADR7y2lZxm9NoVYGNZ2k7reTDuZ4GLbdE4lXZMA8bG3vHLM9ZDYo3XQZ7nXUZa+/FeO12M2ire7gYCHOLf+V/4Xl/hbetiGHT7z5FnzlrtVlG6wQZaZJBn2NdzuLZ4grQHY8jThP8nI=&login_hint=jsmith@example.com&nonce=0.6655489"
    }
}
