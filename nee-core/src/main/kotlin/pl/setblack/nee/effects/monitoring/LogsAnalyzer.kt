...
class LogsAnalyzer {
   fun processLogs(logs: Seq<LogMessage>): LogsReport = logs.foldLeft(InvocationAccumulator()) {
       a, b ->
...
data class FunctionReport(val codeLocation: CodeLocation,
    val invocationCount: Long,
    val totalTime : Long,
    val errorsCount: Long
) {
    operator fun plus(other: FunctionReport)  =
        FunctionReport(this.codeLocation,
